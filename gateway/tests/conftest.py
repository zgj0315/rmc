...
# 「查反向端口在不在」这件事，整个测试套件里只有这一处知道它绑在什么地址上：
# 所有查询都走下面的 reverse_port_registered()，没有一个调用点自己抄地址。
# 唯一的例外是 test_tunnel.py::test_reverse_port_is_bound_on_a_wildcard_address，
# 它是**断言绑定地址本身**的那条用例，故意把三种拼法写死在自己身上——拿这个常量
# 去断言这个常量描述的性质是循环论证，改错了常量它反而会跟着一起绿。
# 方案 7.3 推后的加固要把这些端口从公网挪到内网网卡时，查询侧改这个常量与下面
# 那个函数即可，断言侧还要一并改那条用例（评审记录里也提到，届时它更可能是被
# 删掉而不是被改写）。
