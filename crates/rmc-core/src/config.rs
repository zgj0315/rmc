...
impl Default for Config {
    fn default() -> Self {
        Self {
            // **这个字段今天没有任何生产读取方**：运维服务器的地址、端口、
            // 账号与指纹全部来自连接码（`Command::Start` 带进来），
            // `AppPaths::config()` 只取 `log_dir`。留着它是因为删字段会牵到
            // `validate`/`ValidatedAddresses` 一串，记在
            // `docs/交付前还剩什么.md` 的 B 档，不在文档轮里动。
            //
            // R13-5（修复轮 1/5）：占位值从 `gateway.company.com:443` 换成
            // `203.0.113.10:22000`。旧值是 Task 7 时照「方案设计.md §3.10
            // 界面示意图里的地址框」抄的——**那张示意图已经不存在了**
            //（界面上没有地址框，只有一个连接码框），而那个字面量同时踩中
            // 本项目两条已经拍过板的设计决定：「不接受域名，只用 IP」
            //（spec §4.2）与「不绑 443，默认监听 22000」（spec §3.3）。
            // 一个死字段里躺着两条被明确取消掉的设计，读代码的人无从分辨
            // 它是遗留还是现行规格。换成 IP + 22000 之后，`wording.rs` 那条
            // 唯一的上屏用语豁免（守的就是这个字面量）也一并删掉了。
            // `203.0.113.10` 是 RFC 5737 的文档专用网段，不会指向真实主机。
            gateway: HostPort::new("203.0.113.10", 22000).expect("内置默认值必须自解析通过"),
