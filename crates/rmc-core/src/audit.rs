...
impl Audit {
    ...
    ///
    /// 这不是 `Error::LocalIo` 那套处置——**而本文件自己就是 `LocalIo`
    /// 今天真正的产出点**：`AuditLog::open`（建目录、收紧权限）与 `sweep`
    /// （按保留期删旧文件）失败都判 `LocalIo`/`Fatal`，因为那两件事发生在
    /// 会话之外、重连解决不了，必须让人看见、必须停下来（见 `error.rs` 上
    /// `LocalIo` 的文档）。**同一个文件里两种处置并存，分界就在这里。**
    ///
    /// （Task 13 修复轮 2/5：原注释说这个分类「是为 known_hosts 设计的」，
    /// 而 `knownhosts.rs` 今天只剩指纹工具、零文件读写——那句话在
    /// 修复轮 1 改写 `error.rs` 的 `LocalIo` 文档时就已经自相矛盾了，
    /// 是反向找引用扫出来的：这段注释里那句「见 `error.rs` 上 `LocalIo`
    /// 的文档」正是一条指向被改文档的显式引用。）
    ///
    /// 相比之下，写一行写不进去（目录权限
