//! 主机与端口的解析与校验。
//!
//! 今天的用法：一体机地址（界面上可编辑的那一个）、连接码解析出来的运维
//! 服务器地址、以及代理地址都经此类型。**界面上没有运维服务器的地址框**
//! ——那一半来自连接码，见 `code::ConnectionCode`。
...
///
/// **R13-7（Task 13 修复轮 1/5）：这条约束要留，但理由重写过。**
/// Task 4 立它时举的两个后果都挂在 `known_hosts` 文件上（一个含
/// `]:443 <指纹>\n[…` 的 host 会让一次写入产出两行、第二行是伪造成另一台
/// 服务器的记录；一个含空格的 host 会永久砸坏那个文件）。`knownhosts.rs`
/// 今天**只剩指纹工具，零文件读写**，那两个后果都不存在了——留着会让人
/// 以为这道校验是在守一个已经不存在的文件，进而觉得它可以放宽。
///
/// 今天它守的是这些：
///
/// - **审计日志**（`audit.rs` 一行一条）与诊断包：host 里的 `\n` 会把
///   一条记录劈成两条，后一条完全由输入控制——日志注入，而审计日志正是
///   出事后唯一的凭据；
/// - **HTTP CONNECT 请求行与 `Host:` 头**（`transport/connect.rs`）：
///   host 里的 `\r\n` 就是标准的请求走私/头注入，这一段还常常要穿过
///   客户的代理设备；
/// - **上屏文案与状态卡副标题**：地址会原样出现在里面。
///
/// 字段私有把"不合法的值构造不出来"做成了类型性质，而不是每个调用点
/// 各自记得校验一次。读出口见下面的 `host()`/`port()`。
///
/// （原注释末尾还说"今天树上没有任何结构体字面量构造、也没有生产调用点，
/// 改起来成本是零"——那是 Task 4 时的事实，`HostPort` 现在到处都是。）
